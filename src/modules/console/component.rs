...
impl Component for DevConsole {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            DevConsoleMsg::ClearOutput => {
                self.output_manager.clear();
                self.output_manager.add_output(ConsoleOutput::info("Console cleared"));
                true
            }
            
            DevConsoleMsg::ToggleDebug => {
                let debug_enabled = self.output_manager.toggle_debug();
                let message = if debug_enabled {
                    "Debug messages enabled"
                } else {
                    "Debug messages disabled"
                };
                self.output_manager.add_output(ConsoleOutput::info(message));
                true
            }
        }
    }
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let on_clear = {
            ...
            Callback::from(move |_| {
                link.send_message(DevConsoleMsg::ClearOutput);
            })
        };
        ...
        let on_toggle_debug = {
            let link = ctx.link().clone();
            Callback::from(move |_| {
                link.send_message(DevConsoleMsg::ToggleDebug);
            })
        };
                ...
                <div class="console-modal">
                    <div class="console-header">
                        ...
                        <div class="console-controls">
                            <button class="console-btn" onclick={on_clear} title="Clear output">
                                { "Clear" }
                            </button>
                            <button 
                                class="console-btn" 
                            ...
                            >
                                { if self.output_manager.show_debug() { "Hide Debug" } else { "Show Debug" } }
                            </button>
