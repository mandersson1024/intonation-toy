...
- [ ] Create input module in `/pitch-toy/input/`
- [ ] Define InputManager struct with event dispatcher integration
- [ ] Create input event types in `/pitch-toy/events/input_events.rs`
- [ ] Integrate with existing event system in events/mod.rs
- [ ] Add InputManager to graphics renderer initialization

### Task 2: Canvas Event Handling (AC: 2)
- [ ] Set up mouse event listeners on graphics canvas
- [ ] Implement touch event handling for mobile support
- [ ] Create coordinate transformation from screen to graphics space
- [ ] Add event prevention to avoid browser default behaviors
- [ ] Implement event throttling for performance optimization

### Task 3: Hit Testing System (AC: 2, 3)
- [ ] Create HitTestElement registry for clickable areas
- [ ] Implement hit testing logic for green square in test_scene
- [ ] Add coordinate-based collision detection
- [ ] Register green square as interactive element
- [ ] Implement click detection and logging to web console
