...
fn main() {
    ...
    // Set up appropriate cfg flags for platform abstraction compatibility
    if has_web {
        println!("cargo:rustc-cfg=platform_web");
    } else if has_test_native {
        println!("cargo:rustc-cfg=platform_stubs");
    } else if is_wasm {
        // Default to web platform for wasm targets
        println!("cargo:rustc-cfg=platform_web");
    } else {
        // Default to stubs for native targets
        println!("cargo:rustc-cfg=platform_stubs");
    }
    
    // Ensure compatibility with main intonation-toy package build configuration
    println!("cargo:rustc-cfg=dev_console");
