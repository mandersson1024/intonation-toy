...
use {
    ...
    engine::audio::worklet::create_audio_context_and_load_worklet,
};
...
pub async fn start() {
    ...
    let audio_context = match create_audio_context_and_load_worklet().await {
        Ok(context) => context,
        Err(err) => {
            crate::common::error_log!("Failed to create audio context and load worklet: {:?}", err);
            return;
        }
    };
