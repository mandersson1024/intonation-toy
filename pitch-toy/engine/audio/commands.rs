...
mod tests {
    ...
    fn test_audio_command_execution() {
        ...
        let result = registry.execute("audio status");
        // The result should be a warning about no audio manager, not an error about unknown command
        match result {
            egui_dev_console::ConsoleCommandResult::Output(egui_dev_console::ConsoleOutput::Warning(_)) => {
                // Success - command was found and executed (returned warning about no audio manager)
            },
            ...
            egui_dev_console::ConsoleCommandResult::Output(egui_dev_console::ConsoleOutput::Error(text)) => {
                ...
                // Other errors are acceptable (like "Audio system not initialized")
            },
            _ => {
